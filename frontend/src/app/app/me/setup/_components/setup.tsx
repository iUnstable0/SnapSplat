...
export default function SetupPage({
  ...
  useEffect(() => {
    ...
    if (!issues.setupKey.success) {
      setupKeyRef.current?.focus();
      return;
    }
  ...
  const checkSetupKeyIssues = async () => {
    const setupKey = setupKeyRef.current?.value;

    const setupKeyResult = Z_SetupKey.safeParse(setupKey);

    if (!setupKeyResult.success) {
      let reasons = [];

      if (!setupKey) {
        reasons = ["Setup key is required"];
      } else {
  ...
  const checkFormIssues = async () => {
    ...
    const setupKeyResult = await checkSetupKeyIssues();
          ...
          {blurOverlayOpen && formVisible && (
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  marginTop: issues.confirmPassword.success
                    ? "8px"
                    : calculateIssueMargin(issues.confirmPassword.reasons),
                }}
              >
                <motion.input
                  layout={false}
                ...
                />
                <AnimatePresence mode="popLayout">
                  {!issues.setupKey.success &&
                    issues.setupKey.reasons.map((reason, index) => (
                      <motion.div
                        key={`setupKeyIssues-${index}`}
                        className={styles.inputInvalidText}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                layout
                style={{
                  marginTop: issues.setupKey.success
                    ? "24px"
