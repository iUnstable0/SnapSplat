...
import { Z_Email, Z_Password, Z_DisplayName } from "@/modules/parser";
...
export default async function register(formData: FormData): Promise<void> {
  ...
  const { email, password, confirmPassword, displayName } = sanitizedData.data;
  ...
  try {
    result = (await gql.mutation.register("d", email, password, displayName))
      .register;
  } catch (error) {
